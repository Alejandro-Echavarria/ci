...
<div class="mb-8 justify-end">
    <div class="flex w-full justify-between">
        ...
        <div class="bg-gradient-to-r from-green-200/80 to-green-400/20 rounded-xl text-center cursor-default mt-1 p-1 w-full sm:w-auto">
            <div class="backdrop-blur-sm bg-white/10 rounded-lg focus:outline-none px-5 py-1 h-full">
                <span id="ic" class="text-gray-700 dark:text-gray-200 font-bold text-sm" title="Indice acumulado">Indice acumulado:
                    <span id="ia" class="mx-3 text-sm font-bold text-gray-700 dark:text-gray-200"></span>
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach ($records as $record)
            @php
                ...
                $value = 0;
            @endphp 
            <div class="p-6 rounded-2xl border-2 dark:border-gray-600">
                <div class="mb-5">
                    <div class="flex gap-1">
                        ...
                        <form id="formDelete" class="-my-1 inline-flex ml-auto" method="POST">
                            @csrf
                            @method('delete')
                            <button type="button" class="bg-gray-300/20 hover:bg-gray-300/50 text-gray-700 dark:text-gray-200 rounded-full p-1.5 h-8 w-8 transition-all" data-dismiss-target="#alert" aria-label="Close" onclick="formDelete({{ $record }})">
                ...
                <div class="flex justify-between">
                    ...
                    @if ($record->subjects->count())
                        <div class="hover:bg-gray-300/50 transition rounded-xl text-center cursor-default p-1" title="Indice cuatrimestral">
