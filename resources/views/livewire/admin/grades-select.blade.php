        ...
        @foreach ($subjects as $key =>  $subject)
            <div>
                <div class="flex gap-3">
                    ...
                    <select id="subjects[{{$key}}]" name="subjects[{{$key}}]" wire:model.defer="subjects.{{$key}}" class="w-full p-2.5 text-sm text-gray-700 border-1 dark:border-gray-600 text-white-500 rounded-xl shadow-sm focus:outline-none dark:text-white-400 dark:placeholder:text-white-600 dark:bg-gray-700 dark:text-white transition ease-in-out {{($errors->has('subjects.'.$key) ? ' border-red-400 text-red-900 placeholder-red-700 focus:ring-red-500  focus:border-red-500 dark:text-red-500 dark:placeholder-red-500' : null)}}">
                    ...
                    <input
                        ...
                        type="number"
                        id="credits[{{$key}}]" name="credits[{{$key}}]"
                        class="prevenir-envio w-1/3 p-2.5 text-sm text-gray-700 border-1 text-white-500 rounded-xl shadow-sm border-1 dark:border-gray-600 focus:outline-none dark:text-white-400 dark:placeholder:text-gray-400 dark:bg-gray-700 dark:text-white transition ease-in-out {{($errors->has('credits.'.$key) ? ' border-red-400 text-red-900 placeholder-red-700 focus:ring-red-500  focus:border-red-500 dark:text-red-500 dark:placeholder-red-500' : null)}}"
                        value="{{ old('credits.'.$key) }}"
