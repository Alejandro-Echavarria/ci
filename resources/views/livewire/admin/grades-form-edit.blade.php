<div>
            ...
            @foreach ($subjects as $key =>  $subject)
                ...
                <div>
                    <div class="flex gap-3">
                        ...
                        @else
                            <button type="button" class="text-white focus:outline-none font-medium rounded-xl text-sm w-auto sm:w-auto px-5 py-2.5 text-center red-color cursor-pointer dark:hover:bg-red-300" wire:click.prevent="removeSubject({{ $key }}, {{ $id }})">-</button>
                        @endif           
                    </div>
                    @error('subjects.'. $key)
                        <small class="text-red-500">{{$message}}</small>
    ...
    <div class="w-full flex justify-end">
        ...
        <button wire:click.prevent="save()" type="button" class="text-white color-secundario focus:outline-none font-medium rounded-xl text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:hover:bg-blue-700 cursor-pointer">
            Actualizar
        </button>
    </div>
</div>